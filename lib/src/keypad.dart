...
const double _numberPadRowHeight = 40.0;
...
class _NumberPadGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    final double viewTileHeight = constraints.viewportMainAxisExtent / 4;
    ...
    final List<Widget> numberPads = numberSymbols.map<Widget>((MathSymbol symbol) {
      ...
      Widget pad = Container(
        ...
        child: Text(
          symbol.text,
          style: TextStyle(
            color: isClear ? theme.primaryTextTheme.title.color : Colors.black,
            fontSize: 14.0 * 3.0,
          ),
        ),
      );
      ...
      return FlatButton(
        color: isClear ? theme.primaryColor : Colors.tealAccent,
      ...
      return Expanded(
        child: Container(
          decoration: BoxDecoration(
            border: Border(
              left: BorderSide(width: 3.0, color: Colors.teal ?? theme.dividerColor),
            ),
          ),
          child: opPad,
        ),
  ...
  Widget _createOpSymbolPad(BuildContext context, MathSymbol symbol) {
    ...
    switch (symbol) {
      case MathSymbols.delete:
        ...
        opPad = Text(
          symbol.text,
          style: TextStyle(color: Colors.black,fontSize: fontSize), // theme.primaryTextTheme.title.color, fontSize: fontSize),
        );
        break;
      default:
        opPad = Text(
      ...
      children: <Widget>[
        MaterialButton(
          ...
          child: Icon(
            Icons.undo,
            size: fontSize,
            color: disabledUndo ? disabledIconColor : Colors.black,
          ),
        ),
        MaterialButton(
          shape: shape,
          ...
          child: Icon(
            Icons.redo,
            size: fontSize,
            color: disabledRedo ? disabledIconColor : Colors.black,
          ),
        ),
      ],
    );
